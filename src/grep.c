...
struct grepctx
{
  ...
  char const *lastout;	/* Pointer after last character output;
...
static bool search_file (int, char const *, char const *, bool, bool);
...
/* Return the text type of a file.  BUF, of size SIZE, is the initial
   ...
   which may be arbitrarily written to or read from.  */
static enum textbin
file_textbin (struct grepctx *ctx, char *buf, size_t size, int fd, struct stat const *st)
{
...
fillbuf (struct grepctx *ctx, size_t save, struct stat const *st)
{
  ...
  if (pagesize <= ctx->buffer + ctx->bufalloc - sizeof (uword) - ctx->buflim)
  ...
  else
    {
      ...
      newbuf = ctx->bufalloc < newalloc ? xmalloc (ctx->bufalloc = newalloc) : ctx->buffer;
      ...
      if (SEEK_DATA != SEEK_SET && !ctx->seek_data_failed)
        {
          ...
          if (data_start < 0)
            ...
            {
              ctx->totalnl = add_count (ctx->totalnl, data_start - ctx->bufoffset);
...
static void
print_line_head (struct grepctx *ctx, char const *beg, char const *lim, char sep)
{
...
static const char *
print_line_middle (struct grepctx *ctx, const char *beg, const char *lim,
                                      ...
                                      beg + (cur - beg))) != (size_t) -1))
    {
      ...
      if (match_size == 0)
      ...
      else
        {
          ...
          if (only_matching)
            print_line_head (ctx, b, lim, (out_invert ? SEP_CHAR_REJECTED
                                           : SEP_CHAR_SELECTED));
          else
            {
...
static void
prline (struct grepctx *ctx, char const *beg, char const *lim, char sep)
{
  bool matching;
  ...
  if (!only_matching)
    print_line_head (ctx, beg, lim, sep);
...
static intmax_t
grepbuf (struct grepctx *ctx, char const *beg, char const *lim)
{
...
grep (struct grepctx *ctx, int fd, struct stat const *st)
{
  ...
  if (binary_files == TEXT_BINARY_FILES)
    ...
    {
      textbin = file_textbin (ctx, ctx->bufbeg, ctx->buflim - ctx->bufbeg, fd, st);
      if (textbin_is_binary (textbin))
        {
  ...
  for (;;)
    {
      ...
      if (beg < lim)
        {
          ...
          if (ctx->pending)
            prpending (ctx, lim);
          if ((!ctx->outleft && !ctx->pending) || (nlines && ctx->done_on_match))
            goto finish_grep;
        }
      ...
      if (out_byte)
        ctx->totalcc = add_count (ctx->totalcc, ctx->buflim - ctx->bufbeg - save);
      if (out_line)
        nlscan (ctx, beg);
      if (! fillbuf (ctx, save, st))
        {
      ...
      if (textbin == TEXTBIN_UNKNOWN)
        {
          enum textbin tb = buffer_textbin (ctx->bufbeg, ctx->buflim - ctx->bufbeg);
...
struct search_file
{
  ...
  char *path;
  struct stat st;
  struct search_file *next;
};

static struct
{
  struct search_file *head;
  struct search_file *tail;
  int num_files;
  int producer_done;
...
/* Retrieve a search_file from the work queue, returning NULL if there's
   nothing left to process. */
static struct search_file *
dequeue_search_file (void)
{
  struct search_file *workfile;
...
static void
enqueue_search_file (int fd, char const *path, struct stat *st)
{
  struct search_file *sf;

  sf = xmalloc (sizeof (*sf));
  sf->fd = fd;
  sf->path = xstrdup (path);
  sf->st = *st;
  sf->next = NULL;
  ...
  if (!workqueue.head)
    workqueue.head = workqueue.tail = sf;
  else
    {
      workqueue.tail->next = sf;
      workqueue.tail = sf;
    }
  workqueue.num_files++;
...
worker_thread_func (void *arg)
{
  ...
  while ((file = dequeue_search_file ()))
    {
      ctx.filename = file->path;
      ...
      count = grep (&ctx, file->fd, &file->st);
      status = !count && status;
      if (count_matches)
        {
          lock_output ();
      ...
      if (file->fd == STDIN_FILENO)
        {
          off_t required_offset = ctx.outleft ? ctx.bufoffset : ctx.after_last_match;
          if (required_offset != ctx.bufoffset
              && lseek (file->fd, required_offset, SEEK_SET) < 0
              && S_ISREG (file->st.st_mode))
            suppressible_error (file->path, errno);
        }

      if (file->fd != STDIN_FILENO && close (file->fd) != 0)
        suppressible_error (file->path, errno);
      free (file->path);
      free (file);
    }
...
search_dirent (FTS *fts, FTSENT *ent, bool command_line)
{
  ...
  return search_file (fts->fts_cwd_fd, ent->fts_accpath, name, follow, command_line);
}
...
search_desc (int desc, char const *path, bool command_line)
{
  ...
  if (desc != STDIN_FILENO
      && directories == RECURSE_DIRECTORIES && S_ISDIR (st.st_mode))
    {
      ...
      while ((ent = fts_read (fts)))
        status = search_dirent (fts, ent, command_line) && status;
      if (errno)
        suppressible_error (path, errno);
      if (fts_close (fts))
        suppressible_error (path, errno);
  ...
  enqueue_search_file (desc, path, &st);
             ...
             bool command_line)
{
  ...
  return search_desc (desc, path, command_line);
}
...
{
  if (STREQ (arg, "-"))
    return search_desc (STDIN_FILENO, label ? label : _("(standard input)"), true);
  else
    return search_file (AT_FDCWD, arg, arg, true, true);
}
