...
/* Return the text type of a file.  BUF, of size SIZE, is the initial
   ...
   which may be arbitrarily written to or read from.  */
static enum textbin
file_textbin (struct grepctx *ctx, char *buf, size_t size, int fd,
...
static void
print_line_head (struct grepctx *ctx, char const *beg, char const *lim,
                 char sep)
{
...
grep (struct grepctx *ctx, int fd, struct stat const *st)
{
  ...
  if (binary_files == TEXT_BINARY_FILES)
    ...
    {
      textbin = file_textbin (ctx, ctx->bufbeg, ctx->buflim - ctx->bufbeg, fd,
                              st);
      if (textbin_is_binary (textbin))
        {
  ...
  for (;;)
    {
      ...
      if (textbin == TEXTBIN_UNKNOWN)
        {
          enum textbin tb = buffer_textbin (ctx->bufbeg,
                                            ctx->buflim - ctx->bufbeg);
...
search_dirent (FTS *fts, FTSENT *ent, bool command_line)
{
  ...
  return search_file (fts->fts_cwd_fd, ent->fts_accpath, name, follow,
                      command_line);
}
...
{
  if (STREQ (arg, "-"))
    return search_desc (STDIN_FILENO, label ? label : _("(standard input)"),
                        true);
  else
