...
static int
wchar_context (struct dfa *dfa, wint_t wc)
{
  if (wc == (wchar_t) dfa->syntax.eolbyte || wc == 0)
    return CTX_NEWLINE;
...
dfasyntax (struct dfa *dfa, reg_syntax_t bits, int fold, unsigned char eol)
{
  ...
  dfa->syntax.case_fold = fold != 0;
  dfa->syntax.eolbyte = eol;

  for (i = 0; i < NOTCHAR; ++i)
    {
      dfa->syntax.sbit[i] = char_context (dfa, i);
      switch (dfa->syntax.sbit[i])
        {
        case CTX_LETTER:
          setbit (i, dfa->syntax.letters);
          break;
        case CTX_NEWLINE:
          setbit (i, dfa->syntax.newline);
          break;
        }
    }
}
                  ...
                  if (wc != WEOF && wc2 != WEOF)
                    {
                      ...
                      work_mbc->ranges[work_mbc->nranges].beg
                        = dfa->syntax.case_fold ? towlower (wc) : wc;
                      work_mbc->ranges[work_mbc->nranges++].end
                        = dfa->syntax.case_fold ? towlower (wc2) : wc2;

                      if (dfa->syntax.case_fold && (iswalpha (wc)
                                                    || iswalpha (wc2)))
                        {
                          work_mbc->ranges[work_mbc->nranges].beg
                            = towupper (wc);
      ...
      switch (c)
        ...
        case '`':
          if (backslash && !(dfa->syntax.syntax_bits & RE_NO_GNU_OPS))
            return dfa->lexstate.lasttok = BEGLINE; /* FIXME: should be beginning of string */
          goto normal_char;

        case '\'':
          if (backslash && !(dfa->syntax.syntax_bits & RE_NO_GNU_OPS))
            return dfa->lexstate.lasttok = ENDLINE;   /* FIXME: should be end of string */
          goto normal_char;
  ...
  if (dfa->multibyte && t == MBCSET)
    {
      ...
      if (work_mbc->invert
        ...
        {
          addtok_mb (dfa, MBCSET, ((dfa->nmbcsets - 1) << 2) + 3);
          if (need_or)
            addtok (dfa, OR);
        }
      else
  ...
  if (dfa->parsestate.tok == WCHAR)
          ...
          if (dfa->syntax.case_fold)
            {
              ...
              int i, n = case_folded_counterparts (dfa->lexstate.wctok, folded);
              for (i = 0; i < n; i++)
                {
  ...
  else if (dfa->parsestate.tok == ANYCHAR && using_utf8 ())
    {
      ...
      dfa->parsestate.tok = lex (dfa);
           ...
           || dfa->parsestate.tok == LIMWORD || dfa->parsestate.tok == NOTLIMWORD)
    {
               ...
               wint_t wc, size_t mbclen)
{
  ...
  /* Check syntax bits.  */
  if (wc == (wchar_t) d->syntax.eolbyte)
    {
      if (!(d->syntax.syntax_bits & RE_DOT_NEWLINE))
        return 0;
    }
  else if (wc == (wchar_t) '\0')
    {
      if (d->syntax.syntax_bits & RE_DOT_NOT_NULL)
        return 0;
    }
  else if (wc == WEOF)
    return 0;

  context = wchar_context (d, wc);
               ...
               unsigned char const *end)
{
  ...
  s1 = state_index (d, &d->mb_follows, wchar_context (d, wc));
              ...
              if (s < d->min_trcount)
                {
                  if (d->min_trcount == 1)
                  ...
                  else
                    {
                      ...
                      if (p < mbp)
                        {
                          int context = wchar_context (d, wc);
      ...
      switch (t)
        ...
        default:
          ...
          if (CSET <= t)
            {
              ...
              while (++j < NOTCHAR)
                if (tstbit (j, *ccl)
                    && ! (d->syntax.case_fold && !d->multibyte
                          && toupper (j) == toupper (t)))
                  break;
              if (j < NOTCHAR)
                break;
          ...
          mp->is[0] = mp->left[0] = mp->right[0]
            = d->syntax.case_fold && !d->multibyte ? toupper (t) : t;
